...
@section('content')
  ...
  <div class="container mx-auto py-8 px-4">
    ...
    <div class="bg-white shadow-xl rounded-lg overflow-hidden flex flex-col lg:flex-row">
      ...
      <div class="bg-blue-600 text-white p-6 lg:p-8 flex flex-col items-center lg:w-1/3 xl:w-1/4">
        ...
        <div x-data="{ hovering: false }" @mouseenter="hovering = true" @mouseleave="hovering = false"
          ...
          @php
            $profilePicture = $user->profile_picture
                ...
                : asset('images/profile/default.jpeg');
          @endphp

          <img id="profile_image" src="{{ $profilePicture }}" alt="Foto Profil"
            class="absolute inset-0 w-full h-full object-cover transition-transform duration-300"
            :class="{ 'scale-105': hovering }">

          <div
            class="absolute inset-0 bg-black bg-opacity-60 flex flex-col items-center justify-center text-white
                opacity-0 transition-opacity duration-300"
            x-show="hovering" x-transition.opacity>
            <i class="bi bi-camera-fill text-3xl mb-1"></i>
            <p class="text-xs">Upload Foto</p>
          </div>

          <input x-ref="fileInput" type="file" id="profile_picture_upload" name="profile_picture"
            accept=".jpg,.jpeg,.png" class="hidden" onchange="handleProfilePictureChange(event)">
        </div>
